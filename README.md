...
Utils are ordered **Move**, **Push**, **Pull**, **View**. **View** pans independently of the cursor or centers the viewport. **Move** lifts the current rectangle with an animated border: use directions to position it, Space or Enter to place it, and Escape to cancel without changing the canvas. The cursor stays hidden during the lift and returns at its original on-screen position when the lift ends.
